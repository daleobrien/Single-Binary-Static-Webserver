...
pub fn generate(ctx: &CodegenCtx) {
    ...
    // ── Encoding embedding flags ──
    writeln!(g, "/// Whether gzip-encoded bodies are embedded in the binary.").unwrap();
    writeln!(g, "pub const EMBED_GZIP: bool = {};", ctx.embed_gzip).unwrap();
    writeln!(g, "/// Whether brotli-encoded bodies are embedded in the binary.").unwrap();
    writeln!(g, "pub const EMBED_BROTLI: bool = {};", ctx.embed_brotli).unwrap();
    writeln!(g, "/// Whether zstd-encoded bodies are embedded in the binary.").unwrap();
    writeln!(g, "pub const EMBED_ZSTD: bool = {};", ctx.embed_zstd).unwrap();
    writeln!(g).unwrap();
...
fn write_asset_constants(g: &mut fs::File, ctx: &CodegenCtx) {
    for (i, file) in ctx.files.iter().enumerate() {
        ...
        // When an encoding is disabled at build time, we emit an empty
        // &[u8] instead of include_bytes!, saving the binary size of that
        // variant. The length is set to 0 so the client negotiator skips it.
        let gzip_len_used = if ctx.embed_gzip { gzip_len } else { 0 };
        let brotli_len_used = if ctx.embed_brotli { brotli_len } else { 0 };
        let zstd_len_used = if ctx.embed_zstd { zstd_len } else { 0 };
        ...
        // Conditionally embed encoding variants; disabled => empty slice (0 bytes).
        if ctx.embed_gzip {
            writeln!(
                g,
            ...
            writeln!(g, "const {const_prefix}_BODY_GZIP: &[u8] = b\"\";").unwrap();
        }
        ...
        if ctx.embed_brotli {
            ...
            writeln!(g, "const {const_prefix}_BODY_BROTLI: &[u8] = b\"\";").unwrap();
        }
        ...
        if ctx.embed_zstd {
            ...
            writeln!(g, "const {const_prefix}_BODY_ZSTD: &[u8] = b\"\";").unwrap();
        }

        writeln!(g, "const {const_prefix}_UNCOMPRESSED_LEN: usize = {uncompressed_len};").unwrap();
        writeln!(g, "const {const_prefix}_GZIP_LEN: usize = {gzip_len_used};").unwrap();
        writeln!(g, "const {const_prefix}_BROTLI_LEN: usize = {brotli_len_used};").unwrap();
        writeln!(g, "const {const_prefix}_ZSTD_LEN: usize = {zstd_len_used};").unwrap();
...
fn write_not_found_asset(g: &mut fs::File, ctx: &CodegenCtx) {
    if let Some(ref nf_file) = ctx.not_found_file {
        ...
        writeln!(g, "const NOT_FOUND_TYPE: &str = \"{}\";", utils::mime_for_file(nf_file)).unwrap();
