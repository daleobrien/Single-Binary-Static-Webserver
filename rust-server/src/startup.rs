...
pub(crate) fn print_help() -> ! {
    ...
    elog!("  TCP_HANDLERS_PER_WORKER Number of TCP handler tasks per worker (default: max(MAX_CONNS/WORKERS, 8).min(64))");
    elog!("  H3_HANDLERS_PER_CONNECTION Number of h3 handler tasks per connection (default: 4)");
    elog!("  DISABLE_HTTP3         Compile out all HTTP/3 (QUIC) support (default: true)");
    elog!("  H2_CONN_WINDOW        HTTP/2 connection flow-control window in bytes (default: 1 MiB)");
    elog!("  H2_STREAM_WINDOW      HTTP/2 per-stream flow-control window in bytes (default: 1 MiB)");
    elog!("  H2_MAX_FRAME_SIZE     HTTP/2 max frame size in bytes (default: 16384)");
    elog!("  H2_MAX_SEND_BUF       HTTP/2 per-stream send buffer in bytes (default: 64 KiB)");
    elog!("  SHUTDOWN_TIMEOUT_SECS Graceful shutdown timeout in seconds (default: 3)");
    elog!("  EMBED_GZIP            Embed gzip-encoded asset bodies (default: true)");
    elog!("  EMBED_BROTLI          Embed brotli-encoded asset bodies (default: true)");
    elog!("  EMBED_ZSTD            Embed zstd-encoded asset bodies (default: true)");
